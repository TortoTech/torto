...
| **AI 排版** | 补充未识别的标题、有来源的引用与图表注释；将文内引用收纳为弹窗标记；识别文字和图片公式并渲染。 | ✅ 可选 |
| **引用与链接** | 在同一弹窗中查看脚注和文内引用；统一版式下点击互联网图标即可通过浏览器访问网址。 | ✅ |
...
在**设置 → 排版 → 布局**中选择 AI 排版模型和思考等级。AI 排版补充书籍解析结果：识别以普通段落编写的标题、有明确来源的引用、已识别引用缺少的来源，以及图注和表注，也支持以图片呈现的表格。

文内引用显示为编号标记，每段重新编号。专注模式下，弹窗先列出脚注，再列出文内引用。公式识别同时支持图片公式与文字公式，复制识别后的公式时获得 LaTeX 文本。统一版式还会将网址（包括常见无前缀域名）替换为可点击的互联网图标。

识别优先处理当前阅读区域，按当前小节固定分批。开启翻译时，AI 排版跟随翻译的阅读范围，先准备语义标记再翻译，相关结果一起应用，以减少版面跳动。识别结果可能不准确；公式转换未通过校验时保留原内容。

在对话框中输入 `/` 唤起阅读技能，输入 `@` 添加引用。使用方向键选择提示，按 `Tab` 插入；提示出现时，`Tab` 不会关闭对话框。
...
- AI 排版、翻译、对话和 OCR 使用你配置的服务商。启用后，AI 排版与翻译会随阅读自动发送相关文字或图片，范围可能包含小节上下文或调度范围内尚未读到的内容。
...
- [AI 排版分批策略](docs/ai-layout-batching.md)
- [AI 排版与翻译调度](docs/ai-layout-scheduling.md)
