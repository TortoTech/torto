...
use crate::sentence_byte_ranges;

/// Stable source-node identity for one optional paragraph transformation.
#[derive(Clone, Debug, PartialEq, Eq, Hash)]
pub struct ParagraphStructureKey {
...
/// A frontend-neutral derived publication that presents a selected paragraph
/// as sentence-sized subparagraphs without rewriting its text or source range.
///
/// This layer may wrap an original, translated, or rewritten source. It keeps
/// the canonical node identity intact so layout regeneration and navigation
/// continue to resolve through [`rebook_publication::SourceAnchor`].
pub struct ParagraphStructureSource {
...
impl ParagraphStructureSource {
    ...
    #[must_use]
    pub fn is_active(&self, key: &ParagraphStructureKey) -> bool {
        self.state
            .read()
            .ok()
    ...
    #[must_use]
    pub fn is_structured(&self, key: &ParagraphStructureKey) -> bool {
        self.is_active(key)
    }

    pub fn set_active(&self, key: ParagraphStructureKey, active: bool) -> Result<(), String> {
        self.state
            .write()
    ...
    pub fn can_structure(&self, key: &ParagraphStructureKey) -> Result<bool, String> {
        let section = self
            .inner
            .parse_section(key.section_index)
            .map_err(|error| error.to_string())?;
        let Some(primary) = section.blocks.iter().find_map(|block| match block {
            Block::Text(text)
                if text
                    .source
                    .as_ref()
                    .is_some_and(|range| range.start.node == key.node) =>
            {
                Some(text)
            }
            _ => None,
...
fn paired_punctuation_ranges(chars: &[char]) -> Vec<Range<usize>> {
...
const fn paired_closer(character: char) -> Option<char> {
    match character {
        '“' => Some('”'),
        '‘' => Some('’'),
        '《' => Some('》'),
...
const fn paired_opener(character: char) -> Option<char> {
    match character {
        '”' => Some('“'),
        '’' => Some('‘'),
        '》' => Some('《'),
...
#[cfg(test)]
mod tests {
    use rebook_publication::{BlockStyle, SourceAnchor, SourceRange, SpineItemId, TextStyle};
    ...
    #[test]
    fn sentence_structure_preserves_the_original_source_range() {
        let spine = SpineItemId::new("chapter-1").unwrap();
        let range = SourceRange {
            start: SourceAnchor {
                spine: spine.clone(),
                node: "paragraph-3".into(),
                text_offset: 4,
            },
            end: SourceAnchor {
                spine,
        ...
        assert_eq!(block.source, Some(range));
    }
